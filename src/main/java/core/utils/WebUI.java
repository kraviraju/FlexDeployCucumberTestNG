...
import core.enums.FailureHandling;
import core.helpers.CaptureHelpers;
...
public class WebUI {
    ...
    @Step("Press DOWNARROW keyboard")
    public static boolean pressDOWNARROW() {
        if (ACTIVE_PAGE_LOADED.trim().toLowerCase().equals("true")) {
            waitForPageLoaded();
        }
        sleep(WAIT_SLEEP_STEP);
    ...
    public static void setTextWithSwitch(By by, String str) {
    	WebUI.waitForElementClickable(by, 5);
    	WebUI.clickElementWithJs(by);
    	DriverManager.getDriver().switchTo().activeElement().sendKeys(str);
    }
    ...
    public static void reportTestStep(Scenario scenario,String msg) {
    	WebUI.logConsole("Test Step :"+ msg);
    	//scenario.log("Test Step :"+ msg);
    }
